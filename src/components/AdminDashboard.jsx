...
const AdminDashboard = () => {
    ...
    // Analytics State
    const [viewingAnalyticsUser, setViewingAnalyticsUser] = useState(null);
    const [analyticsPlans, setAnalyticsPlans] = useState([]);
    ...
    const handleViewAnalytics = async (user) => {
        setViewingAnalyticsUser(user);
        setAnalyticsLoading(true);
        setAnalyticsMonth(null);
        try {
            const res = await fetch(`/api/plans/${user.id}`);
            const data = await res.json();
            setAnalyticsPlans(data);
        } catch (error) {
    ...
    const renderAnalyticsModal = () => {
        ...
        if (!selectedMonth) {
            return <p className="analytics-empty">No workout data yet.</p>;
        }
                            ...
                            {sortedTypes.map(([title, count]) => (
                                <div key={title} className="analytics-bar-row">
                                    ...
                                    <div className="analytics-bar-track">
                                        <div className="analytics-bar-fill" style={{ width: `${(count / maxTypeCount) * 100}%` }}></div>
                                    </div>
    ...
    return (
        <div className="dashboard">
            ...
            {/* Analytics Modal */}
            {viewingAnalyticsUser && (
                <div className="modal-overlay">
                    <div className="modal-content dash-card">
                        <div className="modal-header">
                            <h2>Analytics: {viewingAnalyticsUser.username}</h2>
                            <button onClick={closeAnalyticsView} className="btn-close">X</button>
                        </div>
